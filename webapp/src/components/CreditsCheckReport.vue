<template>
  <div class="container-fluid">
    <span class="sec-hdr">Category-wise Earned Credits</span>
    <div class="row mb-2">
      <div class="col">
    ...
    <div class="row mb-2">
      <div class="col">
        ...
        <select id="ctype" class="form-select" v-model.trim="search_crit.course_type">
          <option
            v-for="cs in SD.CourseTypes"
            v-bind:value="cs.id"
            :key="cs.id"
          >
            {{ cs.value }}
      ...
      <div class="col">
        <div>
          ...
          <input
            ...
            class="form-control"
            type="number"
            v-model.trim="search_crit.min_credits"
            min="0"
            max="100"
          />
        </div>
      ...
      <div class="col">
        <div>
          ...
          <input
            ...
            class="form-control"
            type="number"
            v-model.trim="search_crit.max_credits"
            min="0"
            max="100"
          />
        </div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="row row-striped" v-for="(s, i) in credits" :key="s.id">
          ...
          <div class="col-md-2">
            <a :href="'#/std.detail/'+s.user_id">{{ s.entry_no }}</a></div>
          <div class="col-md-4">{{ s.credits }}</div>
...
export default {
  ...
  mounted: function() {
    if(this.$route.name=='credits.check') {
  ...
  methods: {
    is_credit_valid() {
      let vm = this;
      let valid = false;
      if (vm.search_crit.min_credits == "" 
        || vm.search_crit.max_credits == "") {
          valid = true;
      } else if (vm.search_crit.min_credits 
        < vm.search_crit.max_credits) {
          valid = true;
      }
      return valid;
    ...
    search() {
      let vm = this;
      if (!vm.is_credit_valid()) {
        vm.setStatusMessage("Min. credits should be smaller than max. credits. Please correct the inputs!");
        return;
      }
      console.log("Running earned credit checks");
      vm.$http
        .post("earned_credit_check", vm.search_crit)
        .then(function (res) {
          if (res.data.status == "OK") {
            vm.credits = res.data.body.data;
            if(vm.$router.currentRoute.name=='credits.check') {
    ...
    reset() {
      sessionStorage.CCRData = undefined;
      this.credits = [];
      this.search_crit = {
        report_name: "CAT_CREDITS_CHECK",
        acad_session: "-",
