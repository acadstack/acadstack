<!--
Component for Creating  Question wise faculty scores.
-->
<template>
  <div class="container-fluid">
      <span class="sec-hdr">  Question Wise Faculty Feedback Scores: </span>
    <div class="row mb-2">
        <div class="col">
    ...
    <div class="card">
            <div class="card-header">
                ...
                <div class="col-md-1">Instructor Name</div>
        ...
        <div class="card-body">
            <p v-if="quewisefacscore.length == 0">Nothing to show yet!</p>
            <div class="row row-striped" v-for="(s, i) in quewisefacscore" :key="s.id">
            <div class="col-md-1">{{ i + 1 }}</div>
            <div class="col-md-1">{{s.first_name}} {{s.last_name}}</div>
            ...
            <div class="col-md-3">{{s.question}}</div>
            <div class="col-md-1">{{s.q_score}}</div>
...
export default {
  ...
  methods: {    
       search() {
            ...
            vm.$http
                .post("queswise.facscore", vm.form)
                .then(function(res) {
                if (res.data.status == "OK") {
                    vm.quewisefacscore = res.data.body.data;
                    if(vm.$router.currentRoute.name=='queswise.facscore') {
                    let dd = {quewisefacscore: vm.quewisefacscore, form: vm.form};
                    sessionStorage.GenerateCourseEnrolments = JSON.stringify(dd);
                    }
                    vm.setStatusMessage("Found "+vm.quewisefacscore.length+" records");
                } else {
