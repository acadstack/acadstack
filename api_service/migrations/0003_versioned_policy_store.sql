...
-- IF NOT EXISTS guards, so a fresh install where create_schema() already
-- built the current tables can run this file without erroring.


-- ---------------------------------------------------------------- 1.
-- Session ordinal: YYYY-S -> integer, matching acad_session.ordinal()
-- exactly (year * 10 + rank, ranks T1,T2,T3,T4,I,II,S = 0..6).
...
-- row could claim to be effective from 2021-I while sorting as 2019-T1,
-- and every resolution built on the ordinal index would be wrong.
DO $$
BEGIN
...
-- ---------------------------------------------------------------- 3.
-- Resolution reads every version of a group in ordinal order, and both
-- guards below ask for MAX(session_ord). Cover those directly.
CREATE INDEX IF NOT EXISTS policyversion_group_ord_idx
    ON policyversion (policy_group, effective_from_ord);

CREATE INDEX IF NOT EXISTS closedacadsession_ord_idx
    ON closedacademicsession (session_ord);
