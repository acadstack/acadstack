...
def test_the_real_migrations_replay_cleanly(db):
    ...
    exactly that scenario, and the only check that 0001_baseline's
    functions, CHECK constraints and triggers are genuinely idempotent."""
    ...
    # And the immutability machinery still works afterwards. The ordinal
    # comes from acad_session, not a literal, so a hardcoded value here
    # would only be testing that someone remembered to update this line.
